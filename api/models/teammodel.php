...
class TeamModel
{
	...
	function getDashboard()
	{
		...
		if( $this->validator->validate() )
		{
			...
			$withdraw_data = array(
				...
				array('row'=>'Other Income', 'total'=>number_format(0,6), 'withdraw'=>0, 'balance'=>0),
				array('row'=>'Total', 'total'=>number_format( $total_result['referral_income'] + 0 , 6 ), 'withdraw'=>0, 'balance'=>0),
				);
        }
        else
		{
	...
	function hotList()
	{
		...
		$ranksQ = "SELECT users.name, users.country, users.id, ranks.rank FROM users JOIN ranks on ranks.userid=users.id ORDER BY ranks.updated_at DESC LIMIT 30";
