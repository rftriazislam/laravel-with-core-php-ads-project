...
class ApiModel
{
	...
	function getProfileData()
	{
		...
		$my_data = array();
		$this->validator = new Valitron\Validator($_POST);
		$this->validator->rule('required', ['userid', 'password']);
		$this->validator->rule('integer', 'userid');
		$this->validator->rule('lengthMin', 'password', 6);
 ...
 //  	}
	...
	function calculateEarning($todayTotalRef, $todays_icome, $other_income, $referral_all, $others_all)
	{
		$today_sum = $todayTotalRef + $todays_icome['add_valley']+$todays_icome['income_valley']+$todays_icome['earn_more']+$todays_icome['shopping']+$todays_icome['media_and_social']+$todays_icome['weekly_bonus']+$todays_icome['share']+$todays_icome['rank_bonus'];
		$total_sum = $referral_all+$others_all;
		$income_data = array(
			array(
	...
	function loginHelper($reg_id)
	{
		...
		//Get User Income
		$earningTableQ = "SELECT `referral_income`, (`joining_income`+`others_income`) AS other_incomes FROM `earnings` WHERE userid={$reg_id}";
		...
		$income_data = $this->calculateEarning($todayTotalRef, $todays_icome, $other_income, $earningTableData['referral_income'],$earningTableData['other_incomes']);
	...
	function autoLogin()
	{
		...
		$errors = array();
		$this->validator = new Valitron\Validator($_POST);
		$this->validator->rule('required', ['device_id']);
	...
	function registerUser()
	{
		...
		if( $this->validator->validate() )
		{
			...
			{	
				...
				$query = "INSERT INTO users(name, email, password, phonenumber, country, sponsorid, created_at, updated_at) 
				VALUES(:name, :email, :password, :phonenumber, :country, :sponsorid, :created_at, :updated_at)";
